...
#include <math.h>
#include <float.h>
#include "../../3rdParty/eigen-3.3.8/Eigen/Sparse"
#include <iostream>
#include <sstream>
#include "../../3rdParty/eigen-3.3.8/Eigen/IterativeLinearSolvers"
using namespace std;
...
void LSCM::project() {
	...
	typedef Eigen::Triplet<double> T;//稀疏矩阵求解
	...
	for (MeshVertexIterator viter(m_mesh); !viter.end(); viter++) {
		...
		Point p3(p[0], p[1], 0);
		cout << v->id() << endl;
		cout << v->point().x()<< " " <<v->point().y() <<" " <<v->point().z();
		//建立了二维和三维点之间的联系
		map3d.emplace(make_pair(v->id(), v->point()));
		v->point() = p3;
		map2d.emplace(make_pair(v->id(), v->point()));
		cout << v->point().x() << " " << v->point().y() << " " << v->point().z();
		cout << v->id() << endl;
	}
}
